...
export function computeGeodisResult(rows: GeodisRow[], year: number): GeodisResult {
  ...
  // FIX #3 — Livraison "conforme" = avant 12h OU après 14h (exclut le service
  // de midi 12h-14h). Règle confirmée par Nicolas (rapport Krousty).
  const conformesCount = withHour.filter((p) => p.hour <= 12 || p.hour > 14).length;

  // Page "Horaires livraisons" (gabarit compact) : répartition <12h / 12-14h /
  // >14h sur la Messagerie France + "conformes" (= avant 12h OU après 14h,
  // FIX #3 — auparavant le compteur <=48h ouvrées était affiché ici par
  // erreur, d'où le 42 (35%) au lieu de 93 (81,6%)). Null tant que la source
  // ne porte pas d'heure réelle (flux tracking GEODIS actuel : dates à minuit).
  const horaires =
    withHour.length > 0
      ? {
          ...
          conformes_total: withHour.length,
        }
      : null;
  ...
  return {
    ...
    // FIX #3 — % de livraisons conformes (avant 12h OU après 14h).
    respect_horaires_conformes:
      withHour.length > 0 ? round((conformesCount / withHour.length) * 100, 1) : null,
    horaires,
  };
}
