...
function pageArticles(c: Ctx, num = 3) {
  newPage(c);
  pageNumber(c, num);
...
function pageHorairesCompact(c: Ctx, num: number) {
  ...
  const cards: { title: string; value: string }[] = [
    ...
    {
      // FIX #3 — affiché avec 1 décimale (ex. 93 (81,6%)), règle : avant 12h
      // OU après 14h.
      title: "Livraisons conformes",
      value: h ? pctOf(h.conformes, h.conformes_total, 1) : "-",
    },
  ];
...
function renderCompact(c: Ctx) {
  // FIX #1 & #4 — numérotation dynamique et séquentielle : la couverture est
  // la page 1 (non numérotée), puis chaque page incrémente le compteur. Fini
  // les numéros en dur (2, 3, 6, 7, 8, 9) qui faisaient "disparaître" les
  // pages 04/05.
  pageCoverCompact(c);
  let num = 2;
  pageKpiCompact(c, num++);
  pageArticles(c, num++);
  // "SOMMAIRE" affiché sur le panneau Logistique du gabarit B&W (référence).
  pageLogistiqueCompact(c, num++, c.r.client.kpi4 !== "horaire12");
  if (c.r.client.show_horaires) {
    pageHorairesCompact(c, num++);
  }
  pagePerfCompact(c, num++);
  pageFinancesCompact(c, num++);
